...
// this module stays bridge-free so it can be tested without dart types
...
pub fn write_metadata(path: &Path, update: &MetadataWrite<'_>) -> Result<(), String> {
    ...
    tag.set_title(update.title.to_string());
    tag.set_artist(
        std::iter::once(update.primary_artist)
            .chain(update.featured_artists.iter().map(String::as_str))
            .collect::<Vec<_>>()
            .join(" / "),
    );
    tag.set_album(update.album.to_string());
    tag.set_track(update.track_num as u32);
    tag.set_disk(update.disc_num as u32);
    ...
    for artist in std::iter::once(update.primary_artist)
        .chain(update.featured_artists.iter().map(String::as_str))
    {
    ...
    tag.insert_text(ItemKey::AlbumArtist, update.album_artists.join(" / "));
    for artist in update.album_artists {
        tag.push(TagItem::new(
            ItemKey::AlbumArtists,
    ...
    if let Some(artwork_path) = update.artwork_path {
        let mut file =
